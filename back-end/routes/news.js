...
// normaliza qualquer coisa para /uploads/news/<file>
const webPathNews = (p, filename) => {
  if (filename) return `/uploads/news/${filename}`;
  if (!p) return undefined;
  const s = String(p).replace(/\\/g,'/');                 // barra para frente
  const i = s.lastIndexOf('/');                           // pega só o nome
...
router.post('/upload', auth, upload.single('image'), (req, res) => {
  ...
  const web = webPathNews(null, req.file.filename);
  return res.json({
    image: {
      filename: req.file.filename,
      originalName: req.file.originalname,
      path: web, // <- SEMPRE web path
...
router.delete('/image', auth, async (req, res) => {
  try {
    ...
    if (!p) return res.status(400).json({ message: 'Caminho da imagem não informado' });
    // tenta apagar do disco se existir (melhor esforço)
    ...
    try {
      ...
      if (req.file) {
        newsData.image = {
          filename: req.file.filename,
          originalName: req.file.originalname,
          path: webPathNews(null, req.file.filename) // <- aqui
        };
      } else if (req.body.image) {
        const img = parseImageJson(req.body.image);
        if (img) img.path = webPathNews(img.path, img.filename);
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      if (req.file) {
        if (news.image?.path && fs.existsSync(news.image.path)) { try { fs.unlinkSync(news.image.path); } catch {} }
        updateData.image = {
          filename: req.file.filename,
          originalName: req.file.originalname,
          path: webPathNews(null, req.file.filename)
        };
      } else if (updateData.image) {
        const img = parseImageJson(updateData.image);
        if (img) { img.path = webPathNews(img.path, img.filename); updateData.image = img; } else { delete updateData.image; }
      }
